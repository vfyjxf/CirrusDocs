...
ModLauncher 定义了四个 Layer：

```mermaid
flowchart TD
    subgraph 创建顺序
        BOOT["BOOT Layer<br/>（最早）"]
        SERVICE["SERVICE Layer"]
        PLUGIN["PLUGIN Layer"]
        GAME["GAME Layer<br/>（最晚）"]
    end
    BOOT --> SERVICE
    BOOT --> PLUGIN
    SERVICE --> GAME
...
Module Layer 遵循 Java ClassLoader 的隔离规则：parent 无法访问 child 加载的类。PLUGIN 和 SERVICE 的 parent 均为 BOOT，但 PLUGIN 在加载顺序上晚于 SERVICE 创建，层次越高可见的类越多。
...
PLUGIN Layer 在加载顺序上晚于 SERVICE Layer，但两者能访问到的类范围是**相同**的（parent 均为 BOOT）。
:::
