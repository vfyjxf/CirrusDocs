...
Mixin 和 AccessTransformer 的实现基于另一套 API —— `cpw.mods.modlauncher.serviceapi.ILaunchPluginService`。这套 API 面向被 Forge / NeoForge 捆绑的库，与 `ITransformer` 体系不同，`ILaunchPluginService` 能收到**所有** `ClassNode`（而非通过 `targets()` 筛选）。

这也解释了为什么 Mixin 和 AccessTransformer 不属于 CoreMod 范畴——它们走的是独立的 `ILaunchPluginService` 通道，不受 CoreMod 的 Target 和投票机制约束。
...
flowchart TD
    A[开始投票] --> B{存在 YES?}
    B -- 是 --> C[选择一个 YES 执行 transform]
    C --> D{还有 Transformer 剩余?}
    D -- 是 --> A
    D -- 否 --> E[处理完成]
    B -- 否 --> F{存在 DEFER?}
    F -- 是 --> G["DEFERRAL 僵局: 游戏崩溃"]
    F -- 否 --> E
```
